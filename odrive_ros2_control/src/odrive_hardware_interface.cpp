...
struct Axis {
    ...
    double gear_ratio_;         // motor_turns / wheel_turns
    double direction_;          // +1 or -1 per joint (for sign fix)
    ...
    double wheel_torque_target_ = NAN;   // [Nm] wheel-side torque (for ros2_control effort state)
    double wheel_torque_estimate_ = NAN; // optional
...
CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
    ...
    for (auto& joint : info_.joints) {
        auto node_id_str = joint.parameters.at("node_id");
        uint32_t node_id = static_cast<uint32_t>(std::stoul(node_id_str));

        double gear_ratio = 1.0;
        auto it_gr = joint.parameters.find("gear_ratio");
        if (it_gr != joint.parameters.end()) {
            gear_ratio = std::stod(it_gr->second);
        }

        double direction = 1.0;
        auto it_dir = joint.parameters.find("direction");
        if (it_dir != joint.parameters.end()) {
            direction = std::stod(it_dir->second);
        }

        axes_.emplace_back(&can_intf_, node_id, gear_ratio, direction);
    }
...
CallbackReturn ODriveHardwareInterface::on_activate(const State&) {
    ...
    for (auto& axis : axes_) {
        // First send keepalive so watchdog doesn't fire during mode switch
        Set_Input_Vel_msg_t keepalive;
        keepalive.Input_Vel = 0.0f;
        keepalive.Input_Torque_FF = 0.0f;
        axis.send(keepalive);

        // Then set the actual control mode
...
return_type ODriveHardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&) {
    for (auto& axis : axes_) {
        ...
        const double wheel_torque = axis.torque_setpoint_;  // [Nm] wheel joint torque
        const double motor_torque = axis.direction_ * (wheel_torque / axis.gear_ratio_); // [Nm] motor torque
        
        // Send the CAN message that fits the set of enabled setpoints
        if (axis.pos_input_enabled_) {
            Set_Input_Pos_msg_t msg;
            // wheel setpoint [rad] -> motor turns
            double wheel_pos = axis.pos_setpoint_;     // [rad]
            double wheel_vel = axis.vel_setpoint_;     // [rad/s]

            double motor_pos_rad = axis.direction_ * wheel_pos * axis.gear_ratio_;
            double motor_vel_rad = axis.direction_ * wheel_vel * axis.gear_ratio_;

            msg.Input_Pos = motor_pos_rad / (2 * M_PI);          // turns
            msg.Vel_FF    = axis.vel_input_enabled_
                            ? (motor_vel_rad / (2 * M_PI))       // turns/s
                            : 0.0f;
            msg.Torque_FF = axis.torque_input_enabled_ ? motor_torque : 0.0f;
            axis.send(msg);
        } else if (axis.vel_input_enabled_) {
            Set_Input_Vel_msg_t msg;
            double motor_vel_rad = axis.direction_ * axis.vel_setpoint_ * axis.gear_ratio_;
            msg.Input_Vel = motor_vel_rad / (2 * M_PI);
            msg.Input_Torque_FF = axis.torque_input_enabled_ ? motor_torque : 0.0f; // uses outer motor_torque
            axis.send(msg);
        } else if (axis.torque_input_enabled_) {
            Set_Input_Torque_msg_t msg;
            msg.Input_Torque = motor_torque;
            axis.send(msg);
        } else {
            Set_Input_Vel_msg_t msg;
            msg.Input_Vel = 0.0f;
...
void ODriveHardwareInterface::on_can_msg(const can_frame& frame) {
    uint32_t node_id = frame.can_id >> 5;
    uint8_t cmd_id = frame.can_id & 0x1F;

    // Handle heartbeat at interface level (has access to active_)
    if (cmd_id == Heartbeat_msg_t::cmd_id) {
        if (frame.can_dlc >= Heartbeat_msg_t::msg_length) {
            Heartbeat_msg_t msg;
            msg.decode_buf(frame.data);

            if (msg.Axis_Error != 0 && active_) {
                RCLCPP_WARN(
                    rclcpp::get_logger("ODriveHardwareInterface"),
                    "Node %d error detected: 0x%X - clearing and re-arming",
                    node_id, msg.Axis_Error
                );
                for (auto& axis : axes_) {
                    if (axis.node_id_ == node_id) {
    ...
    // Pass all other messages to the correct axis
    for (auto& axis : axes_) {
    ...
    switch (cmd) {
        case Get_Encoder_Estimates_msg_t::cmd_id: {
            if (Get_Encoder_Estimates_msg_t msg; try_decode(msg)) {
                ...
                const double turns_to_rad = 2.0 * M_PI;
                double motor_pos_rad = msg.Pos_Estimate * turns_to_rad;
                double motor_vel_rad = msg.Vel_Estimate * turns_to_rad;

                // Convert to *wheel* angle/vel using gear ratio and direction
                pos_estimate_ = direction_ * (motor_pos_rad / gear_ratio_);
                vel_estimate_ = direction_ * (motor_vel_rad / gear_ratio_);
            }
        } break;
        case Get_Torques_msg_t::cmd_id: {
            if (Get_Torques_msg_t msg; try_decode(msg)) {
                torque_target_ = msg.Torque_Target;       // motor torque
                torque_estimate_ = msg.Torque_Estimate;   // motor torque

                // Convert motor torque -> wheel torque for ros2_control joint effort
                wheel_torque_target_ = direction_ * (torque_target_ * gear_ratio_);
                wheel_torque_estimate_ = direction_ * (torque_estimate_ * gear_ratio_);
            }
        } break;
