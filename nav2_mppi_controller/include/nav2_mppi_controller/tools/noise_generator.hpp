...
protected:
  ...
  /**
   * @brief Generate random controls with mean in control_sequence_.
   * Dispatches to the active SLIP variant (vanilla/log/lowpass/biased).
   *
  ...
  /** @brief vanilla MPPI: i.i.d. Gaussian sampling noise */
  void generateGaussianNoise();

  /** @brief log-MPPI: Gaussian noise scaled by a log-normal factor (heavy tails) */
  void generateLogNormalNoise();

  /** @brief LP-MPPI: Gaussian noise low-pass filtered along the time axis */
  void generateLowPassNoise();

  /** @brief Biased-MPPI: sampling noise for biased samples (mean shift in optimizer) */
  void generateBiasedNoise();
