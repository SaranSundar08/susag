# Amoeba vs vanilla MPPI benchmark
...
Open `benchmark_amoeba.py` and set `START` / `GOAL` to a **straight,
...
Ctrl-C nav2 in terminal A, relaunch with the amoeba params:

```bash
ros2 launch susag_nav2 navigation.launch.py sim:=true \
     nav2_params:=$(ros2 pkg prefix susag_nav2)/share/susag_nav2/param/navigation_amoeba.yaml
...
```bash
python3 benchmark_amoeba.py --report
```
...
- **Headline = `succ%`.** Expect S0 ≈ tie; S1–S3 amoeba higher; S4 amoeba
...
```bash
python3 benchmark_amoeba.py --controller amoeba --reps 2 --scenarios S1_pillar
```
