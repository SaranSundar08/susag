...
#
# TgMppi-vs-vanilla MPPI benchmark harness for nav2 (ROS 2 Humble, Gazebo Classic).
#
# It does NOT swap controllers itself. You run nav2 with ONE params file
# (navigation_sim.yaml = vanilla, navigation_tgmppi.yaml = smart tgmppi),
# tag the run with --controller vanilla|tgmppi, and this script drives the
...
#
#   # then relaunch nav2 with navigation_tgmppi.yaml and:
#   python3 benchmark_tgmppi.py --controller tgmppi --reps 10
#
#   # compare:
#   python3 benchmark_tgmppi.py --report        # prints summary table from the CSV
...
SCENARIOS = {
    # S0: nothing on the path -> tgmppi should ~= vanilla (must not regress badly)
    "S0_open": [],
...
class Bench(Node):
    def __init__(self, controller, reps, scenarios):
        ...
        # Both lidars feed the collision proxy (min range over either); see
        # benchmark_barn.py and PROJECT_STATUS.md 2026-09-09 (no laser merger).
...
def main():
    ...
    ap.add_argument("--controller", choices=["vanilla", "tgmppi"], help="tag for this run")
    ...
    if not args.controller:
        ap.error("--controller is required (vanilla|tgmppi) unless --report")
